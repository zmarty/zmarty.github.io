...
That page is a good example of why this is hard. Three unrelated game reviews share it with no dividing line or gap, and the reading order doesn't match the layout: Cyberia fills the left column on its own, The Horde starts partway down that same left column and continues at the top of the right one, and Battle Bugs sits entirely within the right column, so a naive column-by-column read runs straight from Cyberia into The Horde and then into Battle Bugs, blending three games into one stream. Rating boxes float mid-column and belong to whichever review they follow, which isn't obvious and is easy to get backwards. Body text wraps around screenshots and irregular artwork with no clean border, headlines are drawn rather than typeset (so OCR can't read them at all), and articles span pages with only a small margin tab signalling that page four still belongs to the same walkthrough. Add Romanian diacritics, heavy 1990s slang, and thin white type over dark screenshots, and plain OCR simply gives up.

This problem is document segmentation, a comprehension problem that, across 878 pages, used to mean years of volunteer work, which is why nobody had done it. A vision model looks at the page and works out what it's seeing, and that's what made the project possible. Segmentation also has to pull out everything that isn't text: all 3,991 screenshots need pixel coordinates even though the wrapping text leaves no border to snap to. The winning trick was to draw a labelled grid over the region in page coordinates and read the numbers off it instead of guessing. The bigger lesson was to save those coordinates rather than just the crops: Issue 1's 138 hand-tuned boxes were thrown away and can never be regenerated or upgraded, whereas Issue 16's recorded boxes let all 246 crops be redone from the source PDF with a single command.
